...
import { resolveImportAliases } from '../api';
import type {
  ImportResult,
  MergeDecisions,
  SchemaDecision,
...
export function ProjectImportDialog({ onClose }: Props) {
  ...
  // Alias-confirmation state (format_version=2 imports only)
  // key: `${venue_id}:${alias}` (venue aliases don't contain colons in practice)
  const [aliasDecisions, setAliasDecisions] = useState<Map<string, boolean>>(new Map());
  const [aliasResolved, setAliasResolved] = useState(false);
  const [aliasSubmitting, setAliasSubmitting] = useState(false);

  // Final resolved project (set as soon as the project row is settled, i.e.
  // after upload when no collision, or after collision resolve).
  ...
  // Initialise alias decisions (default: accept all) when pending aliases arrive
  useEffect(() => {
    if (!importResult?.pending_venue_aliases?.length) return;
    const map = new Map<string, boolean>();
    for (const pa of importResult.pending_venue_aliases) {
      map.set(`${pa.venue_id}:${pa.alias}`, true);
    }
    setAliasDecisions(map);
  }, [importResult]);
  ...
  async function handleUpload() {
    ...
    importMut.mutate(file, {
      onSuccess: (result) => {
        ...
        setImporting(false);
        // If no collision, record the settled project ID.
        // The done screen is gated separately on alias resolution.
  ...
  async function handleResolveAliases() {
    if (resolvedProjectId === null) return;
    setError(null);
    setAliasSubmitting(true);
    // Build decisions by iterating the original pending list so we never need
    // to parse the composite map key.
    const decisions = (importResult?.pending_venue_aliases ?? []).map((pa: PendingVenueAlias) => ({
      venue_id: pa.venue_id,
      alias: pa.alias,
      accepted: aliasDecisions.get(`${pa.venue_id}:${pa.alias}`) ?? true,
    }));
    try {
      await resolveImportAliases(resolvedProjectId, decisions);
  ...
  const needsAliasStep =
    !!(importResult?.needs_alias_decision && importResult.pending_venue_aliases.length > 0);
  // Show the alias UI once the project row is settled (resolvedProjectId set)
  // but the user has not yet confirmed aliases.
  const showAliasStep = needsAliasStep && !aliasResolved && resolvedProjectId !== null;
  // The import is truly complete when the project is settled AND either there
  // are no pending aliases or the user has confirmed them.
  const isDone = resolvedProjectId !== null && (!needsAliasStep || aliasResolved);
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
          ...
          {isDone ? (
            ...
            <>
              ...
              {/* ---- Alias confirmation step (format_version=2) ---- */}
              {showAliasStep && (() => {
                // Group pending aliases by venue_name for display
                const byVenue = new Map<string, typeof importResult.pending_venue_aliases>();
                for (const pa of importResult!.pending_venue_aliases) {
                  const group = byVenue.get(pa.venue_name) ?? [];
                  group.push(pa);
                  byVenue.set(pa.venue_name, group);
                }
                const allAccepted = Array.from(aliasDecisions.values()).every(Boolean);
                const allRejected = Array.from(aliasDecisions.values()).every((v) => !v);
                return (
                  <section className="space-y-3">
                    <div className="bg-blue-50 border border-blue-200 rounded p-3 text-sm text-blue-800">
                      <p className="font-medium mb-1">New venue aliases</p>
                      <p>
                        The archive contains{' '}
                        {importResult!.pending_venue_aliases.length} venue alias
                        {importResult!.pending_venue_aliases.length !== 1 ? 'es' : ''}{' '}
                        not in your library. Accept the ones you want added globally.
                      </p>
                    </div>
                    <div className="flex gap-2">
                      <button
                        onClick={() => {
                          const updated = new Map(aliasDecisions);
                          updated.forEach((_, k) => updated.set(k, true));
                          setAliasDecisions(updated);
                        }}
                      ...
                      <button
                        onClick={() => {
                          const updated = new Map(aliasDecisions);
                          updated.forEach((_, k) => updated.set(k, false));
                          setAliasDecisions(updated);
                        }}
                    ...
                    <div className="border border-gray-200 rounded divide-y divide-gray-100 max-h-60 overflow-y-auto">
                      {Array.from(byVenue.entries()).map(([venueName, aliases]) => (
                        <div key={venueName} className="px-3 py-2">
                          <p className="text-xs font-semibold text-gray-700 mb-1">{venueName}</p>
                          <div className="space-y-1">
                            {aliases.map((pa) => {
                              const key = `${pa.venue_id}:${pa.alias}`;
                              const accepted = aliasDecisions.get(key) ?? true;
                              return (
                                <label key={key} className="flex items-center gap-2 cursor-pointer">
                                  <input
                                    ...
                                    onChange={(e) => {
                                      const updated = new Map(aliasDecisions);
                                      updated.set(key, e.target.checked);
                                      setAliasDecisions(updated);
                                    }}
                                    className="rounded"
                                  />
                                  <span className="text-sm text-gray-700 font-mono">{pa.alias}</span>
                                </label>
                              );
                            })}
                          </div>
          ...
          {showAliasStep && (
            <>
              ...
              <button
                ...
                className="px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded hover:bg-blue-700 disabled:opacity-40 disabled:cursor-not-allowed"
              >
                {aliasSubmitting ? 'Saving…' : 'Confirm aliases'}
              </button>
            </>
          )}
        </div>
