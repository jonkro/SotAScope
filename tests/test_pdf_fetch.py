...
class TestFetchPDFFromArxiv:
    def test_success_returns_bytes(self):
        from sotascope.external.pdf_fetch import fetch_pdf_from_arxiv
...
class TestFetchPDFUrlFromUnpaywall:
    def test_oa_available_returns_url(self):
        from sotascope.external.pdf_fetch import fetch_pdf_url_from_unpaywall
...
class TestFetchPDFFromURL:
    def test_success_returns_bytes(self):
        from sotascope.external.pdf_fetch import fetch_pdf_from_url
...
class TestFetchPDFForWork:
    ...
    def test_arxiv_path_succeeds(self):
        from sotascope.external.pdf_fetch import fetch_pdf_for_work

        work = self._make_work(arxiv_id="2401.00001")
        pdf_bytes = b"%PDF-1.4 arxiv"

        with patch("sotascope.external.pdf_fetch.fetch_pdf_from_arxiv", return_value=pdf_bytes) as mock_arxiv:
    ...
    def test_doi_only_path_succeeds(self):
        ...
        with patch("sotascope.external.pdf_fetch.fetch_pdf_from_arxiv") as mock_arxiv, \
             patch("sotascope.external.pdf_fetch.fetch_pdf_url_from_unpaywall", return_value="https://ex.com/p.pdf") as mock_upw, \
             patch("sotascope.external.pdf_fetch.fetch_pdf_from_url", return_value=pdf_bytes) as mock_url:
    ...
    def test_arxiv_fails_then_unpaywall_succeeds(self):
        ...
        with patch("sotascope.external.pdf_fetch.fetch_pdf_from_arxiv", return_value=None), \
             patch("sotascope.external.pdf_fetch.fetch_pdf_url_from_unpaywall", return_value="https://ex.com/p.pdf"), \
             patch("sotascope.external.pdf_fetch.fetch_pdf_from_url", return_value=pdf_bytes):
            result = fetch_pdf_for_work(None, work, email="u@example.com")
    ...
    def test_url_found_but_download_fails_raises(self):
        from sotascope.external.pdf_fetch import fetch_pdf_for_work, PDFFetchError

        work = self._make_work(doi="10.1000/test")

        with patch("sotascope.external.pdf_fetch.fetch_pdf_url_from_unpaywall", return_value="https://ex.com/p.pdf"), \
             patch("sotascope.external.pdf_fetch.fetch_pdf_from_url", return_value=None):
...
def pdf_storage_dir(tmp_path, monkeypatch):
    ...
    storage.mkdir()
    monkeypatch.setattr("sotascope.api.works._get_pdf_root", lambda db: storage)
    return storage
...
_FETCH_FN = "sotascope.external.pdf_fetch.fetch_pdf_for_work"
_SSL_FN = "sotascope.api.enrichment._get_ssl_verify"
_EMAIL_FN = "sotascope.api.enrichment._get_contact_email"
...
class TestFetchPDFEndpoint:
    ...
    def test_download_failure_returns_502(self, client, work_with_arxiv, pdf_storage_dir):
        from sotascope.external.pdf_fetch import PDFFetchError
