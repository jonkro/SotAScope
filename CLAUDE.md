...
- **Discussion page paper selector UX improvements**: `PaperContextSelector` in `DiscussionPage` has three enhancements: (1) selected papers sort to the top alphabetically (unselected appear below), implemented via `useMemo` on `entries`; (2) thin vertical color bars (3 px each, stacked) on each paper row's left edge show which topic list(s) the paper belongs to, using the same colors as the citation timeline legend; (3) a row of compact checkboxes above the paper list — one per topic list — supports bulk select/unselect with native `indeterminate` state (set via refs in `useEffect`) for partially-selected lists. `PaperEntry` now carries `topic_list_ids: number[]` populated from `timeline.seeds`; `topicListColorMap: Map<number, string>` is derived from `timeline.topic_lists`.
